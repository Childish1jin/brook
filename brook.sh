...
config() {
	...
	while :; do
		echo -e "请输入 Brook 密码"
		read -p "$(echo -e "(默认密码: ${cyan}233blog.com$none):")" pass
		[ -z "$pass" ] && pass="233blog.com"
		case $pass in
		*)
			echo
			echo
...
uninstall() {
	if [[ -f /usr/bin/brook ]]; then
		...
		echo -e " \n$red大胸弟...你貌似毛有安装 brook ....卸载个鸡鸡哦...$none\n" && exit 1
	fi
}
try_enable_bbr() {
...
while :; do
	echo
	echo "........... Brook 一键安装脚本 by 233blog.com .........."
	echo
	echo "帮助说明: https://233blog.com/post/35/"
	echo
	echo " 1. 安装"
	echo
	echo " 2. 卸载"
